...
function Result({ weatherData }) { //triggers the animation
  const [reload, setReload] = useState(0)

  const convertUnixTimestamp = (timestamp) => { //converts a Unix timestamp to a date and a time
    const dateObject = new Date(timestamp * 1000) 
    return dateObject.toLocaleString()
  }

  useEffect(() => { //reloads animation after each search
    setReload(prevReload => prevReload + 1)
  }, [weatherData])

  return (
    <div id='datacontainer' key={reload}> 
      {weatherData ? ( //if else condition depending on the data pulled
